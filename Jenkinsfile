node('node-1') {
    ...
    try {
      properties([parameters([choice(choices: ['scripted', 'master', 'declarative'], description: 'choose a branch to build', name: 'BRANCH_TO_BUILD'), string(defaultValue: 'clean package', description: 'specify the maven goal(s)', name: 'MAVEN_GOAL')]), pipelineTriggers([cron('H */3 * * 0,6'), pollSCM('H/10 * * * *')])])  /* you can also use 0 in the place of H */   /* periodical build + pollSCM + parameters*/
      stage('git') {
